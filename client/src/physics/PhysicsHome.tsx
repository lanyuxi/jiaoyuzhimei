...
function PhysicsCoverArtwork({
  coverId,
  kind,
  accent,
}: {
  coverId: string
  kind: PhysicsCoverKind
  accent: string
}) {
  ...
  return (
    <svg viewBox="0 0 520 240" className="h-full w-full" role="img" aria-hidden="true">
      ...
      {kind === 'projectile' && (
        <>
          <path d="M 74 172 C 164 62 288 46 446 174" fill="none" stroke={accent} strokeWidth="4" strokeLinecap="round" />
          <path d="M 108 172 H 446" stroke="#cbd5e1" strokeWidth="2" strokeDasharray="7 7" />
          <circle cx="314" cy="76" r="8" fill="#ef4444" />
          <path d="M 314 76 L 360 76" stroke="#ef4444" strokeWidth="3" strokeLinecap="round" />
        </>
      )}

      {kind === 'pendulum' && (
        <>
          <path d="M 164 56 H 356" stroke="#64748b" strokeWidth="5" strokeLinecap="round" />
          <path d="M 260 58 L 326 166" stroke={accent} strokeWidth="4" />
          <circle cx="326" cy="166" r="20" fill={accent} opacity="0.82" />
          <path d="M 202 176 A 92 92 0 0 0 318 176" fill="none" stroke="#f97316" strokeWidth="3" strokeDasharray="7 7" />
        </>
      )}

      {kind === 'spring' && (
        <>
          <path d="M 92 96 H 160 L 174 72 L 202 120 L 230 72 L 258 120 L 286 72 L 314 120 L 342 72 L 370 96 H 428" fill="none" stroke={accent} strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" />
          <rect x="392" y="78" width="46" height="42" rx="8" fill="#ef4444" opacity="0.82" />
          <path d={physicsWavePath(34, 3.2)} fill="none" stroke="#f97316" strokeWidth="3" opacity="0.82" />
        </>
      )}

      {kind === 'collision' && (
        <>
          <path d="M 112 120 H 210" stroke={accent} strokeWidth="4" strokeLinecap="round" />
          <path d="M 410 120 H 312" stroke="#f97316" strokeWidth="4" strokeLinecap="round" />
          <circle cx="222" cy="120" r="24" fill={accent} opacity="0.82" />
          <circle cx="298" cy="120" r="24" fill="#f97316" opacity="0.82" />
          <path d="M 178 98 L 210 120 L 178 142" fill="none" stroke={accent} strokeWidth="3" />
      ...
      {kind === 'orbit' && (
        <>
          <ellipse cx="260" cy="120" rx="160" ry="64" fill="none" stroke={accent} strokeWidth="4" />
          <ellipse cx="260" cy="120" rx="92" ry="38" fill="none" stroke="#f97316" strokeWidth="3" opacity="0.82" />
          <circle cx="260" cy="120" r="16" fill="#f59e0b" />
          <circle cx="388" cy="83" r="9" fill="#ef4444" />
          <circle cx="174" cy="151" r="7" fill="#2563eb" />
        </>
      )}
        ...
        <>
          {Array.from({ length: 6 }, (_, row) =>
            Array.from({ length: 8 }, (_, col) => {
              const x = 106 + col * 44
              const y = 58 + row * 28
              const dx = Math.cos((row + col) * 0.55) * 13
              const dy = Math.sin((row - col) * 0.45) * 13
              return <path key={`${row}-${col}`} d={`M ${x - dx} ${y - dy} L ${x + dx} ${y + dy}`} stroke={accent} strokeWidth="2.6" strokeLinecap="round" />
            }),
          )}
          <circle cx="194" cy="120" r="16" fill="#ef4444" />
          <circle cx="326" cy="120" r="16" fill="#2563eb" />
        </>
      )}

      {kind === 'wave' && (
        <>
          <path d={physicsWavePath(46, 4)} fill="none" stroke={accent} strokeWidth="4" strokeLinecap="round" />
          <path d={physicsWavePath(26, 7, 1.2)} fill="none" stroke="#f97316" strokeWidth="3" opacity="0.78" />
        </>
      )}

      {kind === 'optics' && (
        <>
          <ellipse cx="260" cy="120" rx="22" ry="88" fill="#dbeafe" stroke={accent} strokeWidth="4" />
          <path d="M 76 78 L 260 120 L 444 78" fill="none" stroke="#ef4444" strokeWidth="3" />
          <path d="M 76 120 H 444" fill="none" stroke={accent} strokeWidth="3" />
          <path d="M 76 162 L 260 120 L 444 162" fill="none" stroke="#f97316" strokeWidth="3" />
        </>
      )}
        ...
        <>
          {Array.from({ length: 11 }, (_, col) =>
            Array.from({ length: 5 }, (_, row) => (
              <rect key={`${col}-${row}`} x={88 + col * 32} y={58 + row * 28} width="30" height="26" rx="5" fill={col < 4 ? '#ef4444' : col < 8 ? accent : '#60a5fa'} opacity={0.32 + (10 - Math.abs(col - 3)) * 0.045} />
            )),
          )}
        </>
      )}
      ...
      {kind === 'fluid' && (
        <>
          <path d="M 76 82 C 154 52 224 108 298 78 C 360 54 410 76 452 60" fill="none" stroke={accent} strokeWidth="4" />
          <path d="M 76 122 C 154 92 224 148 298 118 C 360 94 410 116 452 100" fill="none" stroke="#06b6d4" strokeWidth="4" />
          <path d="M 76 162 C 154 132 224 188 298 158 C 360 134 410 156 452 140" fill="none" stroke="#f97316" strokeWidth="4" />
        </>
      )}

      {kind === 'quantum' && (
        <>
          <path d={physicsWavePath(42, 5)} fill="none" stroke={accent} strokeWidth="4" />
          <path d="M 76 184 C 144 184 152 66 220 66 C 288 66 300 184 376 184 C 414 184 438 156 454 130" fill="none" stroke="#ef4444" strokeWidth="3" opacity="0.82" />
          <circle cx="260" cy="120" r="9" fill="#ef4444" />
        </>
      )}

      {kind === 'relativity' && (
        <>
          <path d="M 96 62 C 176 102 212 142 260 142 C 312 142 350 100 424 62" fill="none" stroke={accent} strokeWidth="4" />
          <path d="M 96 178 C 176 138 212 98 260 98 C 312 98 350 140 424 178" fill="none" stroke="#f97316" strokeWidth="4" />
          <circle cx="260" cy="120" r="28" fill="#111827" opacity="0.85" />
        </>
      )}
    </svg>
  )
    ...
    >
      ...
      <div className="space-y-3 px-4 pb-4 pt-3">
        <div>
          ...
          <p className="mt-3 min-h-[44px] overflow-hidden text-sm leading-[22px] text-[#8a867f] line-clamp-2">{getDescription(experiment)}</p>
        </div>
