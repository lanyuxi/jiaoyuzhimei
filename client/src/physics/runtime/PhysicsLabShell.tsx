...
function TooltipButton({
  ...
  return (
    <button type="button" aria-label={label} title={label} onClick={onClick} className="group relative grid size-10 place-items-center rounded-[6px] border border-[#dedad2] bg-white text-[#4b4742] hover:border-[#165DFF] hover:text-[#165DFF]">
      {children}
...
export default function PhysicsLabShell<TState>({
  ...
  function dispatchSemantic(action: LabAction, detail = action.type) {
    const feedback = controller.reduce(runtime.state, action).feedback
    if (session) recordLabFeedback(repository, session.id, detail, feedback)
  ...
  function completeExperiment() {
    ...
    const completed = completeLabSession(repository, session.id)
    recordLabFeedback(repository, session.id, 'complete-lab', { outcome: 'accepted', message: runtime.completion.message })
  ...
  const apparatus = (
    <section aria-labelledby="lab-apparatus-heading" className="border-b border-[#ece8df] pb-4 lg:border-b-0 lg:border-r lg:pr-4">
  ...
  const steps = (
    <section aria-labelledby="lab-steps-heading" className="border-t border-[#ece8df] pt-4 lg:border-t-0 lg:border-l lg:pl-4">
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-[240px_minmax(0,1fr)_300px]">
        <aside className="hidden lg:block">{apparatus}</aside>
        <details className="border-b border-[#ece8df] pb-3 lg:hidden">
        ...
        <section className="min-w-0">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#ece8df] pb-3">
            ...
            <div className="flex items-center gap-2">
              <TooltipButton label="撤销" onClick={runtime.undo}><Undo2 className="size-4" aria-hidden="true" /></TooltipButton>
              <TooltipButton label="重做" onClick={runtime.redo}><Redo2 className="size-4" aria-hidden="true" /></TooltipButton>
              <TooltipButton label="重置" onClick={runtime.reset}><RotateCcw className="size-4" aria-hidden="true" /></TooltipButton>
              <TooltipButton label="数据表格" onClick={recordCurrentMeasurements}><TableProperties className="size-4" aria-hidden="true" /></TooltipButton>
              <TooltipButton label="实验报告" onClick={() => setReportOpen(true)}><FileText className="size-4" aria-hidden="true" /></TooltipButton>
            </div>
          </div>
          <div className="mt-4 flex aspect-[16/9] min-h-[300px] items-stretch overflow-hidden border border-[#dedad2] bg-[#fdfdfc] lg:min-h-[480px]">
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-[#ece8df] pt-4">
            <button type="button" onClick={recordCurrentMeasurements} className="rounded-[6px] border border-[#165DFF] px-4 py-2 text-sm font-semibold text-[#165DFF]">记录当前读数</button>
            <button type="button" onClick={completeExperiment} className="rounded-[6px] bg-[#165DFF] px-4 py-2 text-sm font-semibold text-white">完成实验</button>
          </div>
        </section>

        <aside className="hidden lg:block">{steps}</aside>
        <details className="border-t border-[#ece8df] pt-3 lg:hidden">
